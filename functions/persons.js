...
const SetProperty = {
  ...
  implementation: (persons, property, value) => {
    ...
    return (
      "Set " + property + " for " + persons.length.toString() + " persons."
    );
  },
};
...
const DeleteProperty = {
  ...
  implementation: (persons, property) => {
    ...
    return (
      "Deleted " + property + " for " + persons.length.toString() + " persons."
    );
  },
};
...
const AddPerson = {
  ...
  implementation: (ctx, wcaUserId, name) => {
    ...
    const existingPerson = ctx.competition.persons.filter(
      (p) => p.wcaUserId == wcaUserId
    )[0];
    if (existingPerson) {
...
const Persons = {
  ...
  implementation: (ctx, filter) => {
    return (ctx.competition.persons || []).filter((person) =>
...
const HasResults = {
  ...
  implementation: (ctx, person) => {
    return ctx.competition.events
      .map((event) => event.rounds[0].results)
      .flat()
      .some(
        (result) =>
          result.personId === person.registrantId && result.attempts.length > 0
      );
  },
};
...
const IsPossibleNoShow = {
  ...
  implementation: (ctx, person) => {
    return (
      ...
      !ctx.competition.events
        .map((event) => event.rounds[0].results)
        .flat()
        .some(
          (result) =>
            result.personId === person.registrantId &&
            result.attempts.length > 0
        ) &&
      ctx.competition.events.some(
        (event) =>
          !event.rounds[0].results
